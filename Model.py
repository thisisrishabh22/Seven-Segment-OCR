...
class Model_Multi(Model):
    ...
    def data_init(self):
        ...
        self.ids_train, self.ids_val, self.y_train, self.y_val = train_test_split(
            self.X, self.y, test_size=0.25, random_state=1)
        self.y_train_vect = [self.y_train["cadran_1"], self.y_train["cadran_2"],
                             self.y_train["cadran_3"], self.y_train["cadran_4"]]
        self.y_val_vect = [self.y_val["cadran_1"], self.y_val["cadran_2"],
                           self.y_val["cadran_3"], self.y_val["cadran_4"]]
    ...
    def model_init(self):
        ...
        x = Conv2D(32, (3, 3), padding='same', name='conv2d_hidden_1',
                   kernel_regularizer=regularizers.l2(0.01))(model_input)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(
            3, 3), name='maxpool_2d_hidden_1')(x)
        x = Dropout(0.30)(x)

        x = Conv2D(64, (3, 3), padding='same', name='conv2d_hidden_2',
                   kernel_regularizer=regularizers.l2(0.01))(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(
            3, 3), name='maxpool_2d_hidden_2')(x)
        x = Dropout(0.30)(x)

        x = Conv2D(128, (3, 3), padding='same', name='conv2d_hidden_3',
                   kernel_regularizer=regularizers.l2(0.01))(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(
            3, 3), name='maxpool_2d_hidden_3')(x)
        x = Dropout(0.30)(x)
        ...
        self.model = keras.models.Model(model_input, outputs)
    ...
    def train(self, lr=1e-3, epochs=50):
        ...
        keras.backend.get_session().run(tf.compat.v1.initialize_all_variables())
        self.history = self.model.fit(self.X_train, self.y_train_vect, batch_size=50,
                                      nb_epoch=epochs, verbose=1, validation_data=(self.X_val, self.y_val_vect))
    ...
    def plot_loss(self):
        ...
        for i in range(1, 5):
            ...
            plt.plot(
                self.history.history['digit_%i_loss' % i], 'r', linewidth=0.5)
            plt.plot(
                self.history.history['val_digit_%i_loss' % i], 'b', linewidth=0.5)
    ...
    def plot_acc(self):
        ...
        for i in range(1, 5):
            plt.figure(figsize=[8, 6])
            plt.plot(
                self.history.history['digit_%i_acc' % i], 'r', linewidth=0.5)
            plt.plot(
                self.history.history['val_digit_%i_acc' % i], 'b', linewidth=0.5)
            plt.legend(
                ['Training Accuracy', 'Validation Accuracy'], fontsize=18)
            plt.xlabel('Epochs ', fontsize=16)
    ...
    def predict(self):
        ...
        for i in range(self.X_val.shape[0]):
            pred_list_i = [np.argmax(pred[i]) for pred in self.y_pred]
            pred_number = 1000 * \
                pred_list_i[0] + 100 * pred_list_i[1] + \
                10 * pred_list_i[2] + 1 * pred_list_i[3]
            val_list_i = self.y_val.values[i].astype('int')
            val_number = 1000 * \
                val_list_i[0] + 100 * val_list_i[1] + \
                10 * val_list_i[2] + 1 * val_list_i[3]
...
class Model_Single(Model):
    ...
    def model_init(self):
        ...
        x = Conv2D(63, (3, 3), padding='same', name='conv2d_hidden_2',
                   kernel_regularizer=regularizers.l2(0.01))(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(
        ...
        self.model = keras.models.Model(model_input, output)
        self.model.make_predict_function()
    ...
    def train(self, lr=1e-3, epochs=5):
        ...
        self.model.compile(loss="sparse_categorical_crossentropy",
                           optimizer=optimizer, metrics=['accuracy'])
        keras.backend.get_session().run(tf.compat.v1.initialize_all_variables())
        self.history = self.model.fit(self.X_train, self.y_train, batch_size=32,
                                      nb_epoch=30, verbose=1, validation_data=(self.X_val, self.y_val))
